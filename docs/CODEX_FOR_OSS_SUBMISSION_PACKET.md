# Codex For OSS Submission Packet

This packet collects the final public repository evidence for the OpenAI Codex
for Open Source form.

Official form: `https://openai.com/form/codex-for-oss/`

Submission status: `READY_FOR_OWNER_SUBMISSION`

Official OpenAI form submission: `NOT_PERFORMED`

OpenAI Organization ID: `OWNER_INPUT_REQUIRED`

## Repository

- GitHub username: `pjmin19`
- Repository URL: `https://github.com/pjmin19/ansimtalk-`
- Maintainer role: `primary maintainer`
- Public status: `PASS: Auto-Verified`
- Latest verified main before M6: `d409d8275abde183b523054f9059452c2c56ada9`
- Official form submission: `NOT_PERFORMED`

## Application Answers

The copy-paste answers are in `docs/openai-codex-for-oss-application.md`.

Current verified character counts:

- `repo_qualification`: `385`
- `api_credits_usage`: `362`
- `anything_else`: `296`

All three are below the 500-character field limit.

## Evidence Checklist

- M0 evidence freeze: `docs/OPENAI_CODEX_FOR_OSS_EVIDENCE.md`, `docs/READINESS_AUDIT.md`
- Automated review readiness: `docs/CODEX_FOR_OSS_AUTOMATED_REVIEW.md`,
  `scripts/check_codex_for_oss_application.py`
- M1 product shape: `docs/ARCHITECTURE.md`, `docs/PRIVACY_BOUNDARIES.md`
- M2 contributor run: `docs/CONTRIBUTOR_LOCAL_RUN.md`, `scripts/generate_sample_report.py`
- M3 domain evaluation: `docs/EVALUATION.md`, `scripts/run_domain_eval.py`
- M4 maintainer automation: `docs/MAINTAINER_AUTOMATION.md`, `scripts/generate_maintainer_report.py`
- M5 human review boundary: `docs/EDUCATOR_GUIDE.md`, `docs/HUMAN_REVIEW_WORKFLOW.md`, `examples/reports/human_review_sample.md`

## Submission Steps

1. Open the official OpenAI form.
2. Enter the GitHub username and repository URL above.
3. Enter the maintainer role as primary maintainer.
4. Paste the three answer blocks from `docs/openai-codex-for-oss-application.md`.
5. Enter the owner-specific OpenAI Organization ID.
6. Submit only after confirming the account-specific fields are correct.

## Boundary

This packet is not a submission receipt and not an acceptance notice. It is not
Human-Verified and not customer-ready. It records that the public repository is
Auto-Verified for owner review and official form entry.
